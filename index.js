...
const { state, saveState } = useSingleFileAuthState(`./metadata/${sessionName}.json`)
...
global.api = (name, path = '/', query = {}, apikeyqueryname) => (name in global.webSer ? global.webSer[name] : name) + path + (query || apikeyqueryname ? '?' + new URLSearchParams(Object.entries({ ...query, ...(apikeyqueryname ? { [apikeyqueryname]: global.APIKeys[name in global.webSer ? global.webSer[name] : name] } : {}) })) : '')

const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })

global.db = new Low(new JSONFile(`metadata/database.json`))
global.db.data = {
    users: {},
    chats: {},
    sticker: {},
...
async function startHisoka() {
    ...
    const hisoka = hisokaConnect({
        ...
        browser: ['Shop Multi Device','Safari','1.0.0'],
        auth: state,
        version
    })
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./message/index")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
	...
	* @param {*} title
     ...
     * @returns
     */
     hisoka.sendListMes = async(jid, text = '', footer = '', title = '', button = '', list = '', quoted = {}) => {
     	const listMessage = {
            text: text,
            footer: footer,
            title: title,
          ...
          hisoka.sendMessage(jid, listMessage, { quoted })
     }
     
     hisoka.sendListMess = async(jid, text = '', footer = '', button = '', list = '', quoted = {}) => {
     	const listMessage = {
            text: text,
            footer: footer,
            buttonText: button,
